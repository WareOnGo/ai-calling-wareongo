...
export async function assignEntities(opts: {
  ...
  return transaction(async client => {
    // Lock entities in deterministic order so concurrent assignment/reassignment is serialized.
    const table = opts.entity === "record" ? "raw_records" : "bolna_call_logs";
    const found = await client.query(`select id from ${table} where id = any($1::uuid[]) order by id for update`, [ids]);
    if (found.rowCount !== ids.length) throw new ApiError(400, "One or more selected records no longer exist");
    const target = await client.query(`select email from bolna_app_users where email = $1 and active for share`, [assignee]);
    if (!target.rowCount) throw new ApiError(400, "Assignee must be an active user");
    if (opts.entity === "record") {
      const missing = await client.query(`select r.id from raw_records r where r.id = any($1::uuid[])
        and not exists (select 1 from raw_phones rp join raw_phone_numbers p using (phone_id)
                        where rp.master_id = r.id and nullif(trim(p.phone), '') is not null) limit 1`, [ids]);
      if (missing.rowCount) throw new ApiError(400, "Select records with a phone number");
    }
    let reassigned = 0;
    if (opts.reassign) {
      ...
      const dropped = await client.query<{ entity_id: string }>(
        `update bolna_assignments set state = 'dropped', completed_at = now(), revision = revision + 1
    ...
    const inserted = await client.query<{ entity_id: string }>(
 ...
 * Ownership and revision are checked together; stale/inaccessible writes conflict.
 * Admins bypass ownership but still supply the expected revision.
 */
...
export async function updateAssignment(
  ...
  if (patch.state === "dropped" && !user.isAdmin) throw new ApiError(403, "Only admins can unassign work");
  const sets: string[] = [];
  const vals: unknown[] = [];
  ...
  vals.push(patch.revision);
  const revisionParam = `$${vals.length}`;
  vals.push(id);
  ...
  const res = await query<AssignmentRow>(
    `update bolna_assignments set ${sets.join(", ")}
      where id = ${idParam}${ownership} and state <> 'dropped' and revision = ${revisionParam}
      returning revision, id, state, outcome, remarks, added_to_db, wh_id`,
    vals,
  );
  if (!res.rowCount) throw new ApiError(409, "Assignment changed or is no longer editable. Refresh to review the latest version.");
  return res.rows[0] ?? null;
}

/** Unassign (admin): close the open assignment without recording an outcome. */
export async function dropAssignment(id: string): Promise<boolean> {
  const res = await query(
    `update bolna_assignments set state = 'dropped', completed_at = now(), revision = revision + 1
...
const HISTORY_FROM = `
  ...
  left join lateral (
    select pp.phone from raw_phones rp
     ...
     where rp.master_id = r.id and nullif(trim(pp.phone), '') is not null order by rp.is_primary desc limit 1
  ) rphone on true
  ...
  left join lateral (
    select bolna_availability(cl2.status, cl2.llm_availability) as llm_availability
      from raw_phones rp2
      join bolna_call_logs cl2 on cl2.phone_id = rp2.phone_id
     where rp2.master_id = r.id
     order by cl2.call_created_at desc nulls last limit 1
  ) rai on true`;

function historyWhere(viewer: import("./scope").Viewer, f: HistoryFilters): { sql: string; params: unknown[] } {
  const where: string[] = [];
  const params: unknown[] = [];
  if (!viewer.isAdmin) {
    params.push(viewer.email.toLowerCase());
    where.push(`a.assignee = $${params.length}`);
  }
...
export async function listAssignments(viewer: import("./scope").Viewer, f: HistoryFilters) {
  const { sql: whereSql, params } = historyWhere(viewer, f);
  const requestedPage = Number.isFinite(f.page) ? Math.max(1, Math.floor(f.page!)) : 1;
  const pageSize = Number.isFinite(f.pageSize) ? Math.max(1, Math.min(100, Math.floor(f.pageSize!))) : HISTORY_PAGE_SIZE;
  ...
  const rowsRes = await query<AssignmentHistoryRow>(
    `select a.revision, a.id::text as id, a.entity_type, a.entity_id::text as entity_id,
            coalesce(r.owner_name, cl.context_details -> 'recipient_data' ->> 'name') as subject,
            coalesce(rphone.phone, case when cl.call_type = 'inbound' then cl.from_number else cl.to_number end) as phone,
            ...
            case when cl.id is not null then bolna_availability(cl.status, cl.llm_availability) else rai.llm_availability end as ai_availability
...
export async function matchingAssignmentTotals(viewer: import("./scope").Viewer, filters: HistoryFilters) {
  const { sql, params } = historyWhere(viewer, filters);
...
export async function assignmentTotals(viewer: import("./scope").Viewer, assignee?: string): Promise<{ open: number; done: number; dropped: number; people: number }> {
  if (!viewer.isAdmin) assignee = viewer.email;
